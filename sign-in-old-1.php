            <!--====== Section 1 ======-->
            <!--<div class="u-s-p-y-60">

                <div class="section__content">
                    <div class="container">
                        <div class="breadcrumb">
                            <div class="breadcrumb__wrap">
                                <ul class="breadcrumb__list">
                                    <li class="has-separator">

                                        <a href="index.php">Home</a></li>
                                    <li class="is-marked">

                                        <a href="index.php?sign-in">Signin</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>-->
            <!--====== End - Section 1 ======-->


            <!--====== Section 2 ======-->
            <div class="u-s-p-b-60">

                <!--====== Section Intro ======-->
                <!--<div class="section__intro u-s-m-b-60">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="section__text-wrap">
                                    <h1 class="section__heading u-c-secondary" style='font-size:140%;'>ALREADY REGISTERED?</h1>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>-->
                <!--====== End - Section Intro ======-->


                <!--====== Section Content ======-->
                <div class="section__content">
                    <div class="container">
                        <div class="row row--center">
                            <div class="col-lg-6 col-md-8 u-s-m-b-30">
                                <div class="l-f-o">
                                    <div class="l-f-o__pad-box">
                                        
                                        <h1 class="gl-h1">SIGNIN</h1>

                                        <span class="gl-text u-s-m-b-30">If you have an account with us, please log in.</span>
                                        <form class="l-f-o__form" method="post" action="#/" onsubmit="login();return false;">
                                            
                                            <div class="u-s-m-b-30">

                                                <label class="gl-label" for="login-email">USERNAME OR E-MAIL *</label>

                                                <input class="input-text input-text--primary-style" type="text" id="username" placeholder="Enter Username / E-mail"></div>
                                            <div class="u-s-m-b-30">

                                                <label class="gl-label" for="login-password">PASSWORD *</label>

                                                <input class="input-text input-text--primary-style" type="password" id="password" placeholder="Enter Password"><!--<span toggle="#password" class="fa fa-fw fa-eye field-icon toggle-password"></span>--></div>
                                            <div class="gl-inline">
                                                <div class="u-s-m-b-30">
                                                    <input type="text" id="result_response" style="display: none;">
                                                    <input type="text" id="redirect_link" style="display: none;">
                                                    <button class="btn btn--e-transparent-brand-b-2" type="submit">LOGIN</button></div>
                                                <!--<div class="u-s-m-b-30">

                                                    <a class="gl-link" href="index.php?lost-password">Lost Your Password?</a></div>-->
                                            </div>
                                            <div class="u-s-m-b-30">

                                                <!--====== Check Box ======-->
                                                <div class="check-box">

                                                    <input type="checkbox" id="remember-me">
                                                    <div class="check-box__state check-box__state--primary">

                                                        <label class="check-box__label" for="remember-me">Remember Me</label></div>
                                                	<br>
                                                	<div class="u-s-m-b-30">

                                                    <a class="gl-link" href="index.php?lost-password">Lost Your Password?</a></div>
                                                </div>
                                                <!--====== End - Check Box ======-->
                                            </div>
                                        </form>
                                        <h1 class="gl-h1">I'M A NEW CUSTOMER / SELLER</h1>

                                        <span class="gl-text u-s-m-b-30">By creating an account with our store, you will be able to move through the checkout process faster, store shipping addresses, view and track your orders in your account and more.</span>
                                        <div class="u-s-m-b-15">

                                            <a class="l-f-o__create-link btn--e-transparent-brand-b-2" href="index.php?sign-up">CREATE AN ACCOUNT</a></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!--====== End - Section Content ======-->
            </div>
            <!--====== End - Section 2 ======-->
